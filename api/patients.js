...
module.exports = async (req, res) => {
    ...
    if (req.method === 'POST') {
        ...
        const { error } = await supabase.from('rawson_pacientes').upsert({
            id, nombre, apellido, historia_clinica, telefono, whatsapp: whatsapp || null, email,
            created_at: created_at || new Date().toISOString(),
            estado_paciente: estado_paciente || 'activo',
            observaciones, medico_derivante_nombre, medico_derivante_telefono,
            medico_derivante_institucion, patologia: patologia || null
        });
        if (error) return res.status(500).json({ error: error.message });
